...
parser.add_argument('-s', action='store', dest='s', required=False, help='the file for basic statistics')
...
with BZ2File(path) as xml_file:
    ...
    for event, element in parser:
        if (element.tag[43:] == 'title'): # lexeme
            ...
            data = extract(lang=par.l, data=element.text) # extraction informations for lexeme
            if not (data is None):
                if not (data[1] == set()):
